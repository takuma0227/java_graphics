public class kadai3dg {
	...
	public static void main(String[] args) {
		...
		Win3D W1 = new Win3D("グーローシェーディング", X_SIZE, Y_SIZE, COLor.WHITE);
	...
	static void paint(Win3D W1, Polygon Pf, Polygon Pn ,Vector li, COLor ca) {
		Vector v = (Vector._sub(Pf.p[0], W1.v0)).unit();	//視線ベクトルの計算
		Vector l = (Vector._sub(Pf.p[0], li)).unit();		//照明ベクトルの計算
		W1.setColor(shading(v,l,Pf.nv,ca));					//描画色の設定
	...
	static COLor sup(COLor c1, COLor c2, double a1, double a2) {
		if(a1+a2==0) return c1;
		return (COLor._add(COLor._mul(a2/(a1+a2), c1), COLor._mul(a1/(a1+a2), c2)));
	}
}
